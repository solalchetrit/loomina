...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-black/5 bg-white/80 backdrop-blur-xl shadow-[0_12px_50px_-36px_rgba(0,0,0,0.35)]">
      <div className="mx-auto flex h-20 max-w-7xl items-center px-5 md:h-24 md:px-10">
        <Link
          href="/"
        ...
        >
          <div className="relative h-12 w-40 md:h-14 md:w-48">
            <Image src="/header_logo.svg" alt="Logo Loomina" fill className="object-contain" priority sizes="12rem" />
          </div>
        ...
        <div className="ml-auto flex items-center gap-3 md:gap-4">
          <nav className="flex items-center gap-2 rounded-full border border-black/5 bg-white/85 px-2 py-1 text-sm font-semibold shadow-sm md:gap-3 md:px-4">
            {NAV_LINKS.map((item) => {
