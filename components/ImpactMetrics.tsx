...
export default function ImpactMetrics() {
  return (
    <section className="w-full bg-[var(--loomina-gray-light)] py-16 px-6 border-y border-[var(--loomina-black)]/10">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10">
          ...
          <p className="text-[var(--loomina-black)] max-w-xl leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {metrics.map((metric) => (
            <div
              key={metric.label}
              className="relative overflow-hidden rounded-2xl p-5 shadow-lg border border-[var(--loomina-black)]/10 bg-white"
            >
              <div className="relative">
                <p className="text-sm text-[var(--loomina-black)] mb-2">{metric.label}</p>
                <p className="text-3xl md:text-4xl font-semibold text-[var(--loomina-black)] leading-tight">
                  {metric.value}
                </p>
                <p className="text-sm text-[var(--loomina-black)] mt-2">{metric.detail}</p>
              </div>
            </div>
          ))}
        </div>
